...
public class GlobalConsumerThread extends Thread {
  ...
  private final Map<String, ConsumeTopicDetails<?,?>> recordHandlers;
  ...
  public GlobalConsumerThread(String name, ConsumerProvider consumerProvider,
                              Map<String, ConsumeTopicDetails<?,?>> recordHandlers,
                              Properties consumerProps) {
    super(name);
    this.recordHandlers = new HashMap<>(recordHandlers);
    this.topics = recordHandlers.keySet();
  ...
  private <K, V> void handleRecord(ConsumerRecord<byte[], byte[]> record) {
    ...
    ConsumeTopicDetails<?, ?> details = recordHandlers.get(record.topic());
    if (details == null) {
      return;
    }
    if (details.getSerializedAction() != null) {
      details.getSerializedAction().apply(record);
    }
    if (details.getDeserializedAction() != null) {
      ...
      details.getDeserializedAction().apply((ConsumerRecord)newRecord);
    }
  }

  @SuppressWarnings("unchecked")
  private <K, V> Topic<K, V> getTopic(String topic) {
    return (Topic<K, V>) Optional.ofNullable(recordHandlers.get(topic))
        .map(ConsumeTopicDetails::getTopic)
        .orElse(null);
  }
